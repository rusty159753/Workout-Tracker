...
import cloudscraper # <--- THE SPECIALIST
...
def execute_cloud_sync():
    ...
    # This automatically mimics a browser and solves JS challenges
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Debug Info if Block Persists
            try:
                page_title = soup.title.string.strip()
            except:
                page_title = "Unknown"
            return {"error": f"Content Blocked. Landed on: '{page_title}'. The firewall is redirecting us."}
...
# Check if cloudscraper is actually installed
try:
    ...
    st.error("🚨 Missing Dependency: `cloudscraper`")
    st.markdown("""
    **Action Required:**
    The standard connection tool is being blocked. You must install the `cloudscraper` library.
    
    **If local:**
    `pip install cloudscraper`
    
    **If on Streamlit Cloud:**
    Add `cloudscraper` to your `requirements.txt` file.
    """)
except Exception as e:
