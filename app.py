...
# --- 2. GLOBAL STYLES (THE NUCLEAR OPTION) ---
st.markdown("""
<style>
    /* Target ALL Streamlit Alert/Info/Warning Containers */
    div[data-testid="stAlert"], div[data-testid="stNotification"] {
        background-color: #172b4d !important; /* Deep Blue */
        border: 1px solid #3b5e8c !important;
        color: #ffffff !important;
    }
    /* RECURSIVE: Force WHITE text on ALL children inside the box */
    div[data-testid="stAlert"] *, div[data-testid="stNotification"] * {
        color: #ffffff !important; 
        fill: #ffffff !important; /* For icons */
    }
    /* Specific overrides for markdown elements to ensure readability */
...
# --- 6. UTILITY: The Context-Aware Janitor ---
def sanitize_text(text):
    ...
    # B. Structural Parsing (The Fix for "Wall of Text")
    soup = BeautifulSoup(text, "html.parser")
    
    # 1. Handle Line Breaks (Lists, BRs) -> Single Newline
    for br in soup.find_all("br"):
        br.replace_with("\n")
        
    # 2. Handle Paragraph Breaks (P, DIV, Header) -> Double Newline
    for block in soup.find_all(['p', 'div', 'h1', 'h2', 'h3', 'h4', 'ul']):
        # Insert newline BEFORE and AFTER to isolate the block
        block.insert_before("\n")
        block.insert_after("\n")

    # 3. Extract Text (Preserving the newlines we just added)
    # separator="" because we manually added \n where we wanted them
    text = soup.get_text(separator="", strip=True)
    
    # C. Cleanup
    text = unicodedata.normalize("NFKD", text)
    
    # D. Markdown Compatibility Logic
    # Streamlit Markdown treats single \n as a space, unless it has 2 spaces before it.
    # But it treats \n\n as a paragraph break.
    # We want to normalize the gaps.
    
    # Collapse 3+ newlines (massive gaps) into 2 (paragraph break)
    text = re.sub(r'\n{3,}', '\n\n', text)
...
def parse_workout_data(wod_data):
    ...
    if isinstance(wod_data, str):
        ...
        # Explicit double newline separation between sections
    ...
    if st.session_state['view_mode'] == 'VIEWER':
        ...
        if not wod or "error" in wod:
            ...
            if "workout" in wod:
                ...
                # --- DISPLAY: FORMATTED & HIGH CONTRAST ---
                # Key Fix: Replace single \n with "  \n" to force Markdown Line Breaks
                # This ensures lists render vertically
